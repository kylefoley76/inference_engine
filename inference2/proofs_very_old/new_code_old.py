import time
import copy
import sys
import pickle
from natural_language_old import step_one






#################### end group


def calculate_time_statistics(num_proved, total_time):
    total_time = time.time() - total_time
    print("")
    print("average " + str("{0:.4f}".format(total_time / num_proved)))
    print("total " + str("{0:.3f}".format(total_time)))
    print("")


def get_result(one_sent, print_type = 4, order = [0, 1], do_not_argue = []):
    total_time = time.time()

    if one_sent != "":
        test_sent = [[0, one_sent]]
    else:
        pkl_file = open('zz_claims_old.pkl', 'rb')
        test_sent = pickle.load(pkl_file)
        pkl_file.close()

    j = -1
    num_proved = 0
    while j < len(order) - 1:
        j += 1
        k = order[j]

        if test_sent[k][0] != 'pass' and k not in do_not_argue:
            num_proved += 1
            st1 = time.time()

            if k == 208:
                bb = 7

            consistent, total_sent = step_one(test_sent[k])
            test_sent[k] = copy.deepcopy(total_sent)

            if print_type != 4:
                if not consistent:
                    if print_type == 3:
                        print(str(k) + " - " + str("{0:.3f}".format(time.time() - st1) + " False"))
                    elif print_type == 0:
                        print (str(k) + " - False")
                        sys.exit()
                elif print_type == 3:
                    print(str(k) + " - " + str("{0:.3f}".format(time.time() - st1)))

    if print_type in [0, 2, 3]:
        if print_type == 0:
            print ("success")
        calculate_time_statistics(num_proved, total_time)

    return test_sent






