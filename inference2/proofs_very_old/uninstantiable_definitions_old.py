...
def main_loop_old(list1, kind="", kind2 = ""):
    m = -1
    while m < len(list1) - 1:
        m += 1
        while list1[m][45] != []:
...
def eliminate_universals(orig_sent, list1, slot):
    ...
    if not ant_done:
        antecedent, antecedentp = put_in_main_loop_old(antecedent, class_sent, "antecedent", word)
    consequent, consequentp = put_in_main_loop_old(consequent, class_sent, "consequent", word)
...
def put_in_main_loop_old(list2, class_sent, kind, word):
    ...
    if list2[45] != [] or kind == 'antecedent':
        if list2[45] != []:
            list2 = main_loop_old([list2], "recursive", kind)
        if kind == 'antecedent':
            if len(list2) > 12:
                list2 = [list2]
