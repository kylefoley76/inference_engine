...
def determine_class(user, dictionary2=""):
    ...
    dictionary = list_of_exc(dictionary)
...
def determine_entailments(by_basicity, dictionary2=[], reduced_def="", output=""):
    ...
    if dictionary2 != []:
        ...
        dictionary.by_basicity = dict(by_basicity)
...
def fill_constituents(oword, new_word):
    if hprop(new_word):
        new_word = new_word[1:]

    if oword[0].isupper():
        new_oword = get_key(dictionary.rel_abbrev, oword)
    else:
        new_oword = oword

    if new_word[0].isupper():
        new_word = get_key(dictionary.rel_abbrev, new_word)
        if new_word != None:
            dictionary.constituents.setdefault(new_oword, set()).add(new_word)
    else:
        dictionary.constituents.setdefault(new_oword, set()).add(new_word)
...
def order_list3(cls, num, level, word_list, p, recent_words, oword):
    ...
    fill_constituents(oword, new_word)

    if dictionary.pos.get(new_word)[:2] == 'nh':
        return level, p
    if new_word == oword:
        print (f"the definition of {oword} is circular")
        for word in recent_words:
            print (word)
        raise Exception
