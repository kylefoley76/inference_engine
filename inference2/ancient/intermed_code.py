...
def get_result(post_data, archive_id=None, request=None, input=None, prove_dict=None):
    ...
    while j < len(order) - 1:
        ...
        truth_value = step_one(test_sent[sent_counter])

        consistent = step_two(truth_value)

        test_sent[sent_counter] = copy.deepcopy(total_sent)
        tot_prop_name.append(prop_name)
        # progress(j+1, len(order))
        if not consistent:
            print(str(sent_counter) + " - " + str("{0:.3f}".format(time.time() - st1) + " False"))
            if proof_type == 0:
                proof_type = 2
                test_sent, row_number = pop_sent()
                # order = [k]
            ...
            print(str(sent_counter) + " - " + str("{0:.3f}".format(time.time() - st1)))
        if mysql == 2:
            if consistent:
                # print('Right')
                result = 'Right'
